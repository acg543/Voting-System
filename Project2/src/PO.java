 ...
 */
public class PO implements VotingType{
     ...
     * @param numSeats
     */
    public PO(String auditFileName, VotingSystem votingSystem, int numSeats){
    ...
    public String auditContent(Vector<Ballot> ballots, Vector<Candidate> candidates, String winner) {
        String votingType = "PO \n";
        String candidateInfo = "Number of Candidates: " + String.valueOf(ballots.firstElement().getVotes().length) + "\n";

        for (Candidate candidate: candidates) {
            candidateInfo += candidate.getName() + " " + String.format("%.2f", (((double) candidate.getNumVotes() / (double) ballots.size())) * 100) + "%" + "\n";
        }
